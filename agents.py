...
class DQNAgent(object):
    ...
    def __init__(self, actor,critic, actor_optimizer,critic_optimizer,replay_buffer,batch_size,replay_start_size, n_act,update_target_steps,explorer,actor_scheduler,critic_scheduler,gamma=0.9):
        ...
        self.actor_pred_func = actor # value function
        self.actor_target_func = copy.deepcopy(actor) # value function
        self.actor_pred_func.to(self.device)
        self.actor_target_func.to(self.device)
        self.actor_optimizer = actor_optimizer  # value function optimizer
        self.actor_scheduler = actor_scheduler  # value function learning rate decay
        self.critic_pred_func = critic # policy function
        self.critic_target_func = copy.deepcopy(critic) # policy function
        self.critic_pred_func.to(self.device)
        self.critic_target_func.to(self.device)
        self.critic_optimizer = critic_optimizer  # policy function optimizer
        self.critic_scheduler = critic_scheduler  # policy function learning rate decay
    ...
    # get action based on experience
    ...
    # get action based on exploration and exploitation
